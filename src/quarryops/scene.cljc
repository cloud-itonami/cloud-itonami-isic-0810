(ns quarryops.scene
  "Bridge from `quarryops.cad`'s tessellated fragment-envelope mesh +
  `quarryops.robotics/simulate-bench-face-settling`'s per-tick physics
  trajectory (`:fragment` body only) into the vertex/index/per-frame-
  transform data shape `kotoba-lang/webgpu`'s `kami.webgpu.mesh`
  executor's REAL, working `upload-mesh!`/`render-frame!` functions
  already consume (ADR-2607995500 -- direct port of `autoparts.scene`'s/
  `fab.scene`'s reference pattern, ADR-2607160000/ADR-2607992500, to
  this vertical's own loose-block free-fall/settling case, itself a
  port of `kami-engine-vehicle-designer`'s `vdesign.scene`,
  ADR-2607151600; see `quarryops.cad`/`quarryops.robotics` docstrings
  for that same porting rationale).

  `:positions`/`:normals`/`:indices` in `scene-for`'s result are
  DIRECTLY the shape `kami.webgpu.mesh/upload-mesh!` destructures
  (`{:keys [positions normals indices uvs morph-target-deltas joints
  weights]}`, all but `:positions`/`:normals`/`:indices` optional) --
  `(select-keys (scene-for extraction) [:positions :normals :indices])`
  is a drop-in `geometry` argument for that function today. `:frames`'s
  per-entry `:transform` map (`{:translation [x y z] :rotation [rx ry
  rz] :scale [sx sy sz]}`) is DIRECTLY the shape `kami.webgpu.mesh/
  model-matrix` (and `render-frame!`'s optional trailing `transform`
  arg, handed straight to `model-matrix`) expects -- one `:frames`
  entry per `quarryops.robotics/simulate-bench-face-settling`
  trajectory tick (`sim-opts` is unused by this ns's own
  `simulate-bench-face-settling`, which takes no tuning opts, unlike
  `autoparts.robotics/run-pull-test`'s/`fab.simphysics/simulate`'s own
  `opts` argument -- kept as an ignored trailing arg only for call-shape
  parity with those siblings, not because this vertical's simulation is
  tunable the same way).

  Two REAL, disclosed gaps close this from being a byte-for-byte
  drop-in -- the SAME two gaps `vdesign.scene`/`autoparts.scene`/
  `fab.scene` close, ported here verbatim because the underlying
  mismatch is identical:

  1. `quarryops.cad/envelope-mesh` produces `{:positions :indices}`
     only -- no `:normals`. `kami.webgpu.mesh/upload-mesh!` requires
     `:normals` (the same length as `:positions` -- a mandatory vertex
     attribute the shader reads, not optional like `:uvs`/skin/morph).
     `face-normals` below computes REAL per-triangle flat normals
     (cross product of each triangle's own two edges) to close this
     gap -- not a placeholder/constant normal.
  2. `quarryops.cad/envelope-mesh`'s positions are in MILLIMETERS
     (`quarryops.cad/envelope-dims-mm` derives `:length-mm`/`:width-mm`/
     `:height-mm`, and `envelope-solid`'s `scale-point` scales the
     base sketch by those mm figures directly) while `quarryops.
     robotics/simulate-bench-face-settling`'s trajectory positions are
     in METERS (`physics-2d` is unit-agnostic, but `quarryops.robotics`
     chose meters -- see `fragment-half-extents-m`, which divides by
     2000.0 to go mm -> half-extent-in-meters). Combining raw mm vertex
     geometry with a meter-scale per-frame translation would place the
     mesh ~1000x too large relative to its own motion. `mesh->m` below
     converts the tessellated positions to meters to close this real
     unit mismatch.

  A THIRD, real, DISCLOSED axis-convention mismatch, genuinely
  different from `autoparts.scene`'s/`fab.scene`'s own two gaps above
  (not present in either sibling, because this vertical's physics uses
  a different world-axis convention): `quarryops.cad/envelope-solid`
  extrudes along CAD-local Z (`quarryops.cad`'s own docstring:
  'sketch on XY ... extrude along Z' -- the SAME convention `autoparts.
  cad`/`fab.cad`/`vdesign.cad` use), but `quarryops.robotics`'s physics
  world is 2D-Y-up (gravity along `[0 -9.81]`, the fragment's fall axis
  is Y, matching `simulate-bench-face-settling`'s own `gravity`). This
  ns does NOT reconcile the two (no rotation is applied to align the
  mesh's Z-extrude axis with the physics world's Y-fall axis) --
  `:frames`' `:transform` below is translation-only, exactly like
  `autoparts.scene`'s/`fab.scene`'s own disclosed 'every frame's
  :rotation is [0 0 0]' limitation (`physics-2d`'s `Body2D` carries NO
  orientation state at all to rotate FROM in the first place). This is
  the SAME disclosed simplification those two ns's already carry, not
  a new one -- worth calling out on its own here only because THIS
  vertical's fall axis (Y) and CAD extrude axis (Z) are visibly
  different world axes, where autoparts'/fab's own pull axis (X) also
  never lined up with their CAD extrude axis (Z) either, just less
  visibly so.

  Same box-footprint-centering property `vdesign.scene`/`autoparts.
  scene`/`fab.scene` document (verified, not assumed, by `scene_test.
  clj` below): `quarryops.cad/envelope-solid` tessellates from `brep.
  feature`'s documented +/-0.5-unit-square sketch, so the footprint is
  ALREADY centered on the local origin in X/Y (min-x = -length/2, max-x
  = +length/2) -- only Z spans `[0,height]` uncentered, the extrude
  direction. No XY shift is needed or applied; only the mm->m unit
  conversion.

  Remaining, honest limitation (same as `vdesign.scene`'s/`autoparts.
  scene`'s/`fab.scene`'s): `kami.webgpu.mesh` itself is a `.cljs`-only
  WebGPU executor (`js/Float32Array`, real GPU device/buffer calls) --
  actually calling `upload-mesh!`/`render-frame!` needs a
  ClojureScript/browser host loading this namespace's output and
  iterating `:frames`, which this JVM-`.cljc` actor repo (no browser
  here) cannot execute, and `kotoba-lang/webgpu` is deliberately NOT a
  runtime dependency of this repo (see deps.edn). The DATA SHAPE this
  namespace produces is genuinely, verifiably compatible with that
  function's real input contract (see `scene_test.clj`); wiring it
  into a live canvas is the small host-side step that remains, and is
  NOT claimed to be done here.

  Also disclosed: every frame's `:rotation` is `[0 0 0]` and `:scale`
  is `[1 1 1]` -- `physics-2d`'s `Body2D` carries NO orientation/
  angular state at all (translation-only rigid body), a real property
  of the underlying solver, not something simplified away by this
  bridge."
  (:require [quarryops.cad :as cad]
            [quarryops.robotics :as robotics]))

(defn- v-sub [[ax ay az] [bx by bz]] [(- bx ax) (- by ay) (- bz az)])

(defn- v-cross [[ax ay az] [bx by bz]]
  [(- (* ay bz) (* az by))
   (- (* az bx) (* ax bz))
   (- (* ax by) (* ay bx))])

(defn- v-length [[x y z]]
  #?(:clj  (Math/sqrt (double (+ (* x x) (* y y) (* z z))))
     :cljs (js/Math.sqrt (+ (* x x) (* y y) (* z z)))))

(defn- flat-normal
  "Real geometric face normal for triangle `a b c` -- cross product of
  two edges, normalized. Falls back to `[0 0 1]` only for a degenerate
  (zero-area) triangle."
  [a b c]
  (let [n (v-cross (v-sub a b) (v-sub a c))
        len (v-length n)]
    (if (pos? len)
      (mapv #(/ % len) n)
      [0.0 0.0 1.0])))

(defn face-normals
  "Per-vertex flat (face) normals for `positions`/`indices`
  (`quarryops.cad/envelope-mesh`'s output shape) -- REAL geometric
  normals computed from each triangle's own 3 vertices, not a
  placeholder or constant. Safe to assign one flat normal per triangle-
  vertex here because `brep.tessellate/tessellate-solid` gives every
  BREP face (this envelope's sketch/extrude box faces) its own PRIVATE
  vertex range -- no vertex index is shared between two faces with
  different normals for this shape, so writing a vertex's normal once
  per triangle it appears in never produces a wrong final value (same
  reasoning `autoparts.scene/face-normals`/`fab.scene/face-normals`/
  `vdesign.scene/face-normals` document, unchanged here)."
  [positions indices]
  (let [tris (partition 3 indices)]
    (reduce
     (fn [normals [ia ib ic]]
       (let [n (flat-normal (nth positions ia) (nth positions ib) (nth positions ic))]
         (-> normals (assoc ia n) (assoc ib n) (assoc ic n))))
     (vec (repeat (count positions) [0.0 0.0 1.0]))
     tris)))

(defn- mesh->m
  "Converts `quarryops.cad`'s millimeter-scale tessellated positions to
  meters, matching `quarryops.robotics`'s meter-scale trajectory
  positions -- see namespace docstring, gap 2. A uniform positive scale
  never changes face-normal directions, so callers may compute `face-
  normals` before or after this conversion; `scene-for` converts
  first, for positions that are already in the same units as
  `:frames`' translations."
  [positions]
  (mapv (fn [[x y z]] [(/ x 1000.0) (/ y 1000.0) (/ z 1000.0)]) positions))

(defn scene-for
  "Builds
    {:positions [...] :normals [...] :indices [...]
     :frames [{:tick n :transform {:translation [x y z]
                                   :rotation [0.0 0.0 0.0]
                                   :scale [1.0 1.0 1.0]}} ...]
     :vertex-count n :index-count n :dims {...}}
  for `extraction` -- the real tessellated fragment envelope
  (`quarryops.cad/envelope-solid`/`envelope-mesh`), unit-converted to
  meters and given real face normals, plus one frame per `quarryops.
  robotics/simulate-bench-face-settling` trajectory tick. `:dims` is
  `quarryops.cad`'s own millimeter-scale `:dims` (`:length-mm`/
  `:width-mm`/`:height-mm`), kept as informational metadata, NOT the
  unit `:positions` is in. `extraction` should be the full extraction
  record. Absent `:fragment-mass-kg`/`:bench-drop-height-m` fall back
  to `quarryops.robotics`'s own disclosed defaults via `quarryops.
  robotics/fragment-for` (the SAME resolution `bench-face-settling-
  telemetry-for` applies -- `quarryops.robotics/simulate-bench-face-
  settling` itself has no bare-mass-number legacy calling convention,
  unlike `autoparts.robotics/run-pull-test`, so this ns must resolve
  defaults before calling it, exactly like `quarryops.robotics`'s own
  `bench-face-settling-telemetry-for` does). `sim-opts` is accepted but
  currently ignored (see ns docstring). See namespace docstring for
  exactly which fields are a direct drop-in for `kami.webgpu.mesh`
  today vs. the disclosed adapter gaps this namespace closes."
  [extraction & [_sim-opts]]
  (let [solid (cad/envelope-solid extraction)
        {:keys [positions indices]} (cad/envelope-mesh solid)
        positions (mesh->m positions)
        normals (face-normals positions indices)
        sim (robotics/simulate-bench-face-settling (robotics/fragment-for extraction))
        frames (mapv (fn [{:keys [tick position]}]
                       {:tick tick
                        :transform {:translation [(first position) (second position) 0.0]
                                    :rotation [0.0 0.0 0.0]
                                    :scale [1.0 1.0 1.0]}})
                     (:trajectory sim))]
    {:positions positions
     :normals normals
     :indices indices
     :frames frames
     :vertex-count (count positions)
     :index-count (count indices)
     :dims (:dims solid)}))
