(ns quarryops.operation
  "OperationActor -- one quarry operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (QuarryOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Quarry Governor (:govern) and the rollout phase
  ...
  One graph run = one quarry operation (intake -> advise -> govern ->
  ...
  decision to a human operator (quarry operator/shift supervisor). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:extraction/extract`/`:consignment/ship` ALWAYS reach
  this node when the governor is clean -- see `quarryops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [quarryops.quarryopsllm :as quarryopsllm]
...
(defn build
  ...
  `quarryops.store/Store`).
  opts:
    :advisor      -- a `quarryops.quarryopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (quarryopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; QuarryOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (quarryopsllm/-advise advisor store request)]
            {:proposal p :audit [(quarryopsllm/trace request p)]})))

      ;; Quarry Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
